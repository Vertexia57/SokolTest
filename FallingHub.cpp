...
void FallingHub::update()
{
	...
	if (colliderData->sideCollissions.down)
	{
		...
		{
			for (int y = floor(colliderData->bounds.top / 32.0f); y < ceil(colliderData->bounds.bottom / 32.0f); y++)
			{
				g_World->setTile(g_TileManager.getTileRef("air"), x, y);
			}
			g_World->setTile(g_TileManager.getTileRef("stone"), x, ceil(colliderData->bounds.bottom / 32.0f));
		}
		HubTileEntity* entity = new HubTileEntity(g_TileManager.getTileEntityRef("passiveGenerator"));
		g_World->addTileEntity(entity, floor(colliderData->bounds.left / 32.0f), ceil(colliderData->bounds.bottom / 32.0f) - 4);
		g_PlayerPointer->setConnectedHub(entity);
	}
}
