	...
	: UIWindow(bounds)
{
	...
	for (int i = 0; i < 4; i++)
		m_BuildSubMenuAnins[i] = m_Animator.addAnimation(lost::EaseMode::OUT, 0.5f);

	m_SubMenuIconAnim = m_Animator.addAnimationChain({
		lost::Animation(lost::EaseMode::OUT, 0.1), 
		lost::Animation(lost::EaseMode::NONE, 0.1), 
		lost::Animation(lost::EaseMode::OUT, 0.3), 
	});

	(*m_SubMenuIconAnim)[0].setStart(1.0f);
	(*m_SubMenuIconAnim)[0].setEnd(0.0f);
	(*m_SubMenuIconAnim)[2].setStart(0.0f);
	(*m_SubMenuIconAnim)[2].setEnd(1.0f);
}
...
void HUDWindow::update()
{
	...
	if (lost::keyTapped(SAPP_KEYCODE_B))
	{
		...
		if (g_PlayerPointer->lockActions && !m_Building)
		{
			...
			m_BuildMenuAnim->setGoal(76.0f * 2.0f);
		}
	}
		...
		if (m_HoveredGroupSlot >= 0 && m_HoveredGroupSlot < buildingGroupCount)
		{
			...
			if (lost::mouseTapped(0))
			{
				...
				if (!m_BuildSubMenuOpen)
				{
					...
					m_BuildSubMenuAnins[1]->setVal(m_Bounds.h - 2 * 76.0f);
					m_BuildSubMenuAnins[2]->setVal(g_TileManager.buildingGroups[m_SelectedGroupSlot].buildingRefs.size() * 76.0f);
					m_BuildSubMenuAnins[3]->setVal(76.0f);
				}
				else
				{
					// Set the end values of the bounds animations
					m_BuildSubMenuAnins[0]->setGoal(
						(m_Bounds.w - buildingGroupCount * 76.0f) / 2.0f + m_SelectedGroupSlot * 76.0f // buildingSlotX
						- ((g_TileManager.buildingGroups[m_SelectedGroupSlot].buildingRefs.size() - 1) * 76.0f / 2)); // offset
					m_BuildSubMenuAnins[1]->setGoal(m_Bounds.h - 2 * 76.0f);
					m_BuildSubMenuAnins[2]->setGoal(g_TileManager.buildingGroups[m_SelectedGroupSlot].buildingRefs.size() * 76.0f);
					m_BuildSubMenuAnins[3]->setGoal(76.0f);
				}
...
void HUDWindow::render()
{
	...
	if (m_Building || m_BuildMenuAnim->getVal() < 76.0f * 2.0f)
	{
		int buildingGroupCount = g_TileManager.buildingGroups.size();
		...
		if (m_BuildSubMenuOpen)
		{
			...
			sgp_draw_filled_rect(m_BuildSubMenuAnins[0]->getVal(), m_BuildSubMenuAnins[1]->getVal() + m_BuildMenuAnim->getVal(), m_BuildSubMenuAnins[2]->getVal(), m_BuildSubMenuAnins[3]->getVal());

			if ((*m_SubMenuIconAnim)[1].getComplete())
			{
				...
				for (int i = 0; i < buildingCount; i++)
				{
					...
					sgp_set_color(1.0f, 1.0f, 1.0f, (*m_SubMenuIconAnim)[2].getVal());
					lost::useImage(buildingRef->texture);
					sgp_draw_textured_rect(0,
						{ (m_Bounds.w - buildingCount * 76.0f) / 2.0f + 6.0f + i * 76.0f + (32.0f - buildingRef->width * 64.0f * scale / 2.0f) - (buildingGroupCount - 1) * 76.0f / 2.0f + m_SelectedGroupSlot * 76.0f, m_Bounds.h - 70.0f - 76.0f + m_BuildMenuAnim->getVal(), buildingRef->width * 64.0f * scale, buildingRef->height * 64.0f * scale },
						{ 0.0f, 0.0f, imageWidth, imageHeight }
					);

					//lost::clearImage();
					//lost::renderTextPro(std::to_string((int)buildingRef->cost),
					//	{ (m_Bounds.w - buildingCount * 76.0f + 76.0f) / 2.0f + i * 76.0f - (buildingGroupCount - 1) * 76.0f / 2.0f + m_SelectedGroupSlot * 76.0f, m_Bounds.h - 74.0f - 76.0f + m_BuildMenuAnim->getVal() }, 1.0f,
					//	LOST_TEXT_ALIGN_MIDDLE, LOST_TEXT_ALIGN_BOTTOM);
				}
			}
			if (!(*m_SubMenuIconAnim)[0].getComplete() && m_OldGroupSlot != -1)
			{
